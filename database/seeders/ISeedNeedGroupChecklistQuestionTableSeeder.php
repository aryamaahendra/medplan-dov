...
class ISeedNeedGroupChecklistQuestionTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if (config('database.default') === 'pgsql') {
            \DB::statement("SELECT setval(pg_get_serial_sequence('need_group_checklist_question', 'id'), coalesce(max(id), 1), max(id) IS NOT null) FROM need_group_checklist_question;");
        }
    }
}
